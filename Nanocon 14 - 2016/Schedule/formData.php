...
function formalizeData(){
	...
	for ($i = 0; $i < $numberEvents; $i++) {
		...
		$startTime = strtotime($formData[$i][37], date("m/d/y"));
		...
		$tmpEvents[$i]['eventVenue'] = $formData[$i][16];
		
		// Next we need the primary  event tag
		$tmpEvents[$i]['primaryTag'] = $formData[$i][9];
		...
		// Head Count - The number of people expected to play
		$tmpEvents[$i]['headCount'] = $formData[$i][3];
		
		// With that, we want the list of moderators		
		$tmpEvents[$i]['moderatorList'] = $formData[$i][13];
		...
		$tmpEvents[$i]['eventDescription'] = $formData[$i][3];
...
function schedEventImport($eventList){
	...
	for($i=0; $i<$numberEvents; $i++){
		...
		// Head Count - The number of people expected to play - None this year
		$tmpEvents[$i][7] = $eventList[$i]["headCount"];
		...
		$tmpEvents[$i][15] = $eventList[$i]["eventVenue"];
